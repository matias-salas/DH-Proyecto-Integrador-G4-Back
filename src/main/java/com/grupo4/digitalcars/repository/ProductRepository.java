...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "SELECT P.* FROM product P WHERE P.category_id = ?1 AND P.id NOT IN (SELECT DISTINCT R.product_id FROM reservation R WHERE (R.checkout_date >= ?2 AND R.check_in_date <= ?3))", nativeQuery = true)
    List<Product> findByCategoryAndDateRange(Category categoryId, LocalDate startDate, LocalDate endDate);
