...
public class ProductController {
    ...
    @GetMapping("/searchByCategoryAndDateRange/{categoryId}/{startDate}/{endDate}")
    public ResponseEntity<List<Product>> searchProductsByCategoryAndDateRange(
            @PathVariable Category categoryId, 
            @PathVariable @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate startDate, 
            @PathVariable @DateTimeFormat(pattern = "yyyy-MM-dd") LocalDate endDate) {
    
        List<Product> productsSearch = prodctService.getProductsByCategoryAndDateRange(categoryId, startDate, endDate);
        if (!productsSearch.isEmpty()) {
            return ResponseEntity.ok(productsSearch);
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
